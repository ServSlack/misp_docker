...
# Change PHP Settings:
sudo sed -i 's/^\(upload_max_filesize\) = .*$/\1 = 150M/g' /etc/php/8.3/apache2/php.ini
sudo sed -i 's/^\(post_max_size\) = .*$/\1 = 50M/g' /etc/php/8.3/apache2/php.ini
sudo sed -i 's/^\(max_execution_time\) = .*$/\1 = 300/g' /etc/php/8.3/apache2/php.ini
sudo sed -i 's/^\(memory_limit\) = .*$/\1 = 8196M/g' /etc/php/8.3/apache2/php.ini
sudo sed -i 's/^\(session\.sid_length\) = .*$/\1 = 32/g' /etc/php/8.3/apache2/php.ini
sudo sed -i 's/^\(session\.use_strict_mode\) = .*$/\1 = 1/g' /etc/php/8.3/apache2/php.ini
sudo sed -i 's/^\(session\.save_handler\) = .*$/\1 = redis/g' /etc/php/8.3/apache2/php.ini
sudo sed -i '0,/.*session.save_path.*/s/.*session.save_path.*/session.save_path = "tcp:'\\/''\\/'localhost:6379?database=12"/' /etc/php/8.3/apache2/php.ini
